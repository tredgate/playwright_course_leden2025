...
import { LoginPageFluent } from "../fluent/login_page_fluent";
import { AtomicLoginPage } from "./atomic_login_page";

export class AtomicHomePage {
  readonly page: Page;
  ...
  async waitOnPage(): Promise<AtomicHomePage> {
    await expect(this.contentTitle).toBeVisible();
    await expect(this.profileImage).toBeVisible();
    return this;
  }

  async clickProfile(): Promise<AtomicHomePage> {
    await this.profileButton.click();
    return this;
  }

  async clickLogoff(): Promise<AtomicLoginPage> {
    await this.logOffButton.click();
    return new AtomicLoginPage(this.page);
  }
}
