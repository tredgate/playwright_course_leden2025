...
import { AtomicHomePage } from "../pages/atomic/atomic_home_page";
import { AtomicLoginPage } from "../pages/atomic/atomic_login_page";

test.describe("Homepage Atomic Tests", () => {
  test.beforeEach(async ({ page }) => {
    const loginPage = await new AtomicLoginPage(page);
    await loginPage
      .openPmtool()
  ...
  test("Top bar checks", async ({ page }) => {
    const homepage = await new AtomicHomePage(page);
